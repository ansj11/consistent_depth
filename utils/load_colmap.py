...
def extrinsics_to_images(extrinsics):
    ...
    for i, extr in enumerate(extrinsics):
        R, t = extr[:, :3], extr[:, -1:]
        Rc = ROT_COLMAP_TO_NORMAL.dot(R.T).dot(ROT_COLMAP_TO_NORMAL.T) # x是[x,-y,-z]的点，位姿是[x,y,z]的，先变回R坐标系旋转，在回到当前坐标系
        tc = -Rc.dot(ROT_COLMAP_TO_NORMAL.T).dot(t) # 保证坐标系一样, t也要改变符号,刚好抵消后面的矩阵相乘
