...
def pixels_to_rays(pixels, intrinsics):
    ...
    cs = principal_point(intrinsics, (H, W)) # cx, cy
    # Convert to [-(W-1)/2, (W-1)/2] x [-(H-1)/2, (H-1)/2)] and bottom left is (0, 0)
    uvs = pixels - cs.view(-1, 2, 1, 1) # u - cx, v - cy
    uvs[:, 1] = -uvs[:, 1]  # flip v
...
def project(points, intrinsics):
    ...
    uvs = rays[:, :2] * fxys.view(-1, 2, 1, 1) # x = X/Z*f + cx
...
def reproject_points(points_cam_ref, extrinsics_ref, extrinsics_tgt):
    ...
    R_ref = extrinsics_ref[..., :p_dim] # 旋转矩阵B,3,3
    t_ref = extrinsics_ref[..., -1:] # B, 3, 1
    points_world = torch.baddbmm(t_ref, R_ref, points_cam_ref.view(B, p_dim, -1)) # 原点坐标

    # Reproject to target:
    # R'^T * (p - t') where t' of (B, 3, 1), R' of (B, 3, 3) and p of (B, 3, H*W)
    R_tgt = extrinsics_tgt[..., :p_dim]
    t_tgt = extrinsics_tgt[..., -1:]
    points_cam_tgt = torch.bmm(R_tgt.transpose(1, 2), points_world - t_tgt) # 目标视角坐标
    #set_trace()
    return points_cam_tgt.view(-1, p_dim, H, W)
