...
class Net(nn.Module):
    def __init__(self, output_channel=1, semantic_channel=2, rate=1):
        ...
        #rgb+depth
        self.layer0_guide = nn.Sequential( conv_bn(5, c[0], 2),
                                           conv_dw(c[0], c[1], 1))
        self.layer1_guide = nn.Sequential( conv_dw(c[1], c[2], 2),
                                           conv_dw(c[2], c[2], 1))
        self.layer2_guide = nn.Sequential( conv_dw(c[2], c[3], 2),
                                           conv_dw(c[3], c[3], 1))
        self.layer3_guide = nn.Sequential( conv_dw(c[3], c[4], 2),
                                           conv_dw(c[4], c[4], 1))
        self.layer4_guide = nn.Sequential( conv_dw(c[4], c[5], 2),
                                           conv_dw(c[5], c[5], 1))    # top layer1
        #rgb
        ...
        self.se1_guide = SELayer(c[1])
        self.se2_guide = SELayer(c[2])
        self.se3_guide = SELayer(c[3])
        self.se4_guide = SELayer(c[4])
        self.se5_guide = SELayer(c[5])
        ...
        self.predictl0_guide = predict(c[1], self.output_channel)
        self.predictl1_guide = predict(c[2], self.output_channel)
        self.predictl2_guide = predict(c[3], self.output_channel)
        self.predictl3_guide = predict(c[4], self.output_channel)
        self.predictl4_guide = predict(c[4], self.output_channel)

    def forward(self, x):

        ######################################################
        # Bottom-up
        c1_guide = self.layer0_guide(x)
        #c1_guide = self.se1_guide(c1_guide)
        pre1_guide = self.predictl0_guide(c1_guide)
        pre1_guide = self.up(pre1_guide)

        c2_guide = self.layer1_guide(c1_guide)
        #c2_guide = self.se2_guide(c2_guide)
        pre2_guide = self.predictl1_guide(c2_guide)
        pre2_guide = self.up4(pre2_guide)

        c3_guide = self.layer2_guide(c2_guide)
        #c3_guide = self.se3_guide(c3_guide)
        pre3_guide = self.predictl2_guide(c3_guide)
        pre3_guide = self.up8(pre3_guide) 

        c4_guide = self.layer3_guide(c3_guide)
        #c4_guide = self.se4_guide(c4_guide)
        pre4_guide = self.predictl3_guide(c4_guide)
        pre4_guide = self.up16(pre4_guide)

        c5_guide = self.layer4_guide(c4_guide)
        #c5_guide = self.se5_guide(c5_guide)
        ######################################################
        ...
        # Top-down
        p4 = self.upconv1(c5_guide)# (256,8,8)
        pre5_guide = self.predictl4_guide(p4)
        pre5_guide = self.up32(pre5_guide)
        ...
        #p = self.mnconv5(p)
        return self.predict(p),pre1,pre2,pre3,pre4,pre1_guide,pre2_guide,pre3_guide,pre4_guide,pre5_guide,F.normalize(norm),semantic



def calc_computation(scene_model):
    torch.set_grad_enabled(False)
    _inputs = Variable(torch.rand((2, 5, IMAGE_HEIGHT, IMAGE_WIDTH)))
