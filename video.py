...
class Video:
    ...
    def check_extracted_pts(self):
        pts_file = "%s/frames.txt" % self.path # results/*/frames.txt
        if not os.path.exists(pts_file):
            return False
        with open(pts_file, "r") as file:
            lines = file.readlines()
            self.frame_count = int(lines[0]) # 帧数
            width = int(lines[1])
            height = int(lines[2]) # 原图宽高
            ...
            print("frames.txt exists, checked OK.") # 行数==帧数+3(num, w, h)
            return True
        return False
    ...
    def extract_frames(self):
        frame_dir = "%s/color_full" % self.path # results/*/color_full
    ...
    def downscale_frames(
        ...
        down_dir = pjoin(self.path, subdir) # results/*/color_down
