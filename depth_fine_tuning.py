...
class DepthFineTuningParams:
    ...
    def add_arguments(parser):
        ...
        parser.add_argument("--learning_rate", type=float, default=0, # 自动设置lr
...
class DepthFineTuner:
    def __init__(self, range_dir, frames, params):
        ...
        if "our" in params.model_type:
            self.model = get_depth_model(params.model_type)
        else:
    ...
    def save_depth(self, dir: str = None, frames=None, reload=False):
        if dir is None:
            dir = self.out_dir
        if frames is None:
            frames = self.frames

        if 'our' in self.params.model_type:
            color_fmt = pjoin(self.base_dir, "color_down_png", "frame_{:06d}.png")
        else:
            color_fmt = pjoin(self.base_dir, "color_down", "frame_{:06d}.raw")
        depth_dir = pjoin(dir, "depth")
        depth_fmt = pjoin(depth_dir, "frame_{:06d}")

        dataset = VideoFrameDataset(color_fmt, frames, self.params.model_type)
        ...
        if reload:
            ...
            print('Reload model state dict')
        torch.backends.cudnn.enabled = True
        torch.backends.cudnn.benchmark = True

        self.model.eval()
        
        loss, last_depth = 0, None
        os.makedirs(depth_dir, exist_ok=True)
        for i, data in enumerate(data_loader):
            data = to_device(data)
            stacked_images, metadata = data
            frame_id = metadata["frame_id"][0]
            gt = np.nan_to_num(metadata['depth'].cpu().numpy().squeeze()) if 'depth' in metadata else None
            if 'our128' in self.params.model_type:
                B, C, H, W = stacked_images.size()
                #stacked_images = F.interpolate(stacked_images.view(-1, C, H, W), (tH, tW), mode='bilinear')
                depth = self.model(stacked_images)[0]
                #depth = F.interpolate(depth, (H, W), mode='bilinear')
            elif 'our256' in self.params.model_type or 'our320' in self.params.model_type:
                B, C, H, W = stacked_images.size()
                #stacked_images = F.interpolate(stacked_images.view(-1, C, H, W), (tH, tW), mode='bilinear')
                guide = metadata['guide'] if 'guide' in metadata else stacked_images[:,:2] * 0
                #guide[:,0] = last_depth * guide[:,1] if last_depth is not None else guide[:,0]
                input = torch.cat([stacked_images, guide], dim=1)
                if '320' not in self.params.model_type:
                    depth = self.model(input)[0]
                else:
                    depth = self.model(input)
                depth = F.interpolate(depth, (H, W), mode='bilinear').view(B, -1, H, W)
                last_depth = depth.clone().detach()
                depth = torch.clamp(depth, 0.05, 100)
            else:
                depth = self.model.forward(stacked_images, metadata)
            # set_trace()
            depth = depth.detach().cpu().numpy().squeeze()
            loss += mae(depth, gt, (gt>0).astype('float32')*(gt<20).astype('float32')) if gt is not None else 0
            inv_depth = 1.0 / depth if self.is_inv else depth
            #ix = np.isinf(inv_depth) | (inv_depth < 0)
            #inv_depth[ix] = float("nan")
            print(loss / (i+1), depth.max())

            image_io.save_raw_float32_image(
                depth_fmt.format(frame_id) + ".raw", inv_depth)
        
        with open(pjoin(self.base_dir, '%s-loss.txt'%self.params.model_type), 'a') as f:
    ...
    def fine_tune(self, writer=None):
        ...
        dataset = VideoDataset(self.base_dir, meta_file, self.params.model_type)
        ...
        for epoch in range(self.params.num_epochs):
            ...
            for data in train_data_loader:
                ...
                if 'our128' in self.params.model_type:
                    B, _, C, H, W = stacked_img.size()
                    #stacked_img = F.interpolate(stacked_img.view(-1, C, H, W), (tH, tW), mode='bilinear')
                    try:
                        depth = self.model(stacked_img)[0]
                    except:
                        continue
                    depth = F.interpolate(depth, (H, W), mode='bilinear').view(B, -1, H, W)
                    depth = torch.clamp(depth, 0.05, 100)
                    ...
                    B, _, C, H, W = stacked_img.size()
                    #stacked_img = F.interpolate(stacked_img.view(-1, C, H, W), (tH, tW), mode='bilinear')
                    guide, gt = metadata['guide'], metadata['depth']
                    input = torch.cat([stacked_img, guide], dim=2)
                    if '320' not in self.params.model_type:
                        depth = self.model(input)[0]
                    else:
                        depth = self.model(input)
                    depth = F.interpolate(depth, (H, W), mode='bilinear').view(B, -1, H, W)
                    loss_gt = torch.abs(depth - gt.squeeze()).mean()
                    depth = torch.clamp(depth, 0.05, 100)
                else:
                ...
                loss, loss_meta = criterion(
                    depth, metadata, parameters=self.model.parameters(), images=stacked_img)

                pairs = metadata['geometry_consistency']['indices']
                pairs = pairs.cpu().numpy().tolist()

                print(f"Epoch = {epoch}, pairs = {pairs}, loss = {loss[0]}, loss_gt = {loss_gt}, max={depth.max()}")
                if torch.isnan(loss):
    ...
    def eval_and_save(self, criterion, data_loader, suf) -> Dict[str, torch.Tensor]:
        ...
        for _, data in zip(range(N), data_loader):
            ...
            inv_depths_batch = 1.0 / depth if self.is_inv else depth
            ix = np.isinf(inv_depths_batch) | (inv_depths_batch < 0)
            inv_depths_batch[ix] = float("nan")
