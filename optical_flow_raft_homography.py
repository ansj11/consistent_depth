...
def parse_args():
    ...
    parser.add_argument(
        ...
        type=str,
        default="./checkpoints/models/raft-things.pth",
    )
...
def infer(args, raft, device, img1_name, img2_name):
    ...
    if args.homography != 1:
        sz = img1.size()
        img1_view, img2_view = img1, img2
        flow, flow2 = raft(img1_view, img2_view, iters=20, test_mode=True)
        flow = F.interpolate(flow, scale_factor=8, mode='bilinear') * 8
        flow2 = F.interpolate(flow2, scale_factor=8, mode='bilinear') * 8
        flow = flow[0].permute(1, 2, 0).data.cpu().numpy()
        flow2 = flow2[0].permute(1, 2, 0).data.cpu().numpy()
    else:
        sz = img1.size()
        img1_view, img2_reg_view = img1, img2_reg
        flow, flow2 = raft(img1_view, img2_reg_view, iters=20, test_mode=True)
        flow = F.interpolate(flow, scale_factor=8, mode='bilinear') * 8
        flow2 = F.interpolate(flow2, scale_factor=8, mode='bilinear') * 8
        flow = flow[0].permute(1, 2, 0).data.cpu().numpy()
        flow2 = flow2[0].permute(1, 2, 0).data.cpu().numpy()
        ...
        fxx = copy.deepcopy(fx) + flow[:, :, 0] # +offset
...
def process(args):
    ...
    raft = RAFT(args)
    print(f"Loading pretrained model from '{args.pretrained_model_raft}'.")
    raft_ckpt = torch.load(args.pretrained_model_flownet2)
    raft_ckpt = {k.replace('module.', ''): v for k, v in raft_ckpt.items()}
    raft.load_state_dict(raft_ckpt)
    raft.to(device)
    raft.eval()
    ...
    for im1, im2, out in zip(args.im1, args.im2, args.out):
        ...
        flow = infer(args, raft, device, im1, im2)
