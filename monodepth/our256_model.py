...
from .MobileNetv1Quant3 import Net
from .depth_model import DepthModel


class Our256Model(DepthModel):
    ...
    def __init__(self, support_cpu: bool = False, pretrained: bool = True):
        ...
        try:
            self.model = torch.load('/home/anshijie/2021/consistent_depth/checkpoints/model-best500.model')
            print('load 500M model from .model')
        except:
            self.model = Net(rate=2)
            ckpt = torch.load('/home/anshijie/2021/consistent_depth/checkpoints/model-best.pth')
            self.model.load_state_dict(ckpt)
            print('load 500M model from .pth')
        # 兼容1.6
