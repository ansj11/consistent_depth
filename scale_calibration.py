...
def calibrate_scale(video, out_dir, frame_range, args):
    ...
    dense_dir = colmap.dense_dir(colmap_dir, 0) # */colmap_dense/dense/0
    #set_trace()    
    if os.path.isfile(src_meta_file):
        print("Checked metadata file exists.")
    else: # metadata.npz不存在，colmap计算位姿
    ...
    ) # */depth_colmap_dense/depth/frame_%06d.raw

    # convert colmap dense depths to .raw
    converted_depth_dir = os.path.dirname(converted_depth_fmt) # */depth_colmap_dense/depth
    dense_depth_dir = pjoin(dense_dir, "stereo", "depth_maps") # */colmap_dense/stereo/depth_maps
    ...
    if not check_frames(
        ...
        ) # */colmap_dense/dense/0/stereo/depth_maps/frame_{:06d}.png.geometric.bin
        for i in frames:
            colmap_depth_fn = colmap_depth_fmt.format(i) # *bin文件
            ...
            inv_cmp_depth = 1.0 / cmp_depth if args.is_inv else cmp_depth
    ...
    if (
        ...
        src_to_colmap_scales = np.loadtxt(scales_file, delimiter=',') # 存在scales则加载
        ...
        for i in frames:
            converted_depth_fn = converted_depth_fmt.format(i) # .raw
            ...
            # mvs 补全专用,深度可能全0
            if inv_cmp_depth.max() != 0:
                scales = (inv_src_depth / inv_cmp_depth)[inv_cmp_depth != 0]
